...
// Функция инициализации структуры односвязного списка на базе переданной структуры:
struct node* init(struct door* door) {
    struct node* my_node = (struct node*)malloc(sizeof(struct node));  // создаем новый элемент списка

    if (my_node != NULL) {
        my_node->data.id = door->id;          // запись данных в поле id
        my_node->data.status = door->status;  // запись данных в поле status
        my_node->next = NULL;
    }
...
struct node* find_door(int door_id, struct node* root) {
    ...
    while (p_node != NULL) {  // пока не дойдем до конца списка
        ...
        p_node = p_node->next;  // перемещение к следующей двери
    }
...
void destroy(struct node* root) {
    ...
    while (p_node != NULL) {
        ...
        p_node = p_node->next;  // переход к следующему элементу

        free(deleted_node);  // удаление элемента
    }
}
